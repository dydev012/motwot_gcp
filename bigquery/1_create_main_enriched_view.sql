CREATE OR REPLACE VIEW `motwot_v2.motwot_main_enriched` AS
SELECT 
  *,
  ARRAY_LAST(motTests).completedDate AS last_test_date,
  COALESCE(ARRAY_LAST(motTests).testResult, 'NEVER MOT') AS last_test_result,
